...
export function StatCard({ icon, label, value, compact = false }: StatCardProps) {
  return (
    <div
      className={`rounded-[20px] border border-[#353535] bg-[#222222] ${
    ...
    >
      ...
      >
        <i className={`${icon} text-white ${compact ? 'text-sm' : 'text-lg sm:text-xl'}`} />
      </div>
      <p
        className={`font-extrabold leading-none tracking-tight text-white ${
          compact ? 'mt-1.5 text-lg' : 'mt-3 text-2xl sm:mt-4 sm:text-[34px]'
        }`}
      >
        {value}
      </p>
      <p
        className={`mt-0.5 font-medium text-[#8F8F8F] ${
          compact ? 'text-[10px]' : 'mt-1 text-[13px] sm:text-[15px]'
        }`}
      >
        {label}
      </p>
