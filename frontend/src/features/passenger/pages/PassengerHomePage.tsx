      ...
      <div className="px-12 py-[30px]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">{currentTrip ? 'Listo para tu viaje de hoy?' : 'Busca un viaje para hoy'}</p>
          </div>
        ...
        <div className="mt-[22px] grid gap-5" style={{ gridTemplateColumns: '1.3fr 1fr' }}>
          {currentTrip ? (
            <Card inverted className="p-4 flex flex-col justify-between cursor-pointer hover:opacity-95 transition-opacity" onClick={() => navigate('/pasajero/buscar')}>
              <div className="flex items-center justify-between">
                ...
                <b className="text-sm">{currentTrip.departureTime?.slice(0, 5) ?? '—'}</b>
              </div>
              ...
              <div className="mt-2.5 flex items-center">
                <Button variant="dark" size="sm" style={{ backgroundColor: '#141414', color: '#fff' }} onClick={(e) => { e.stopPropagation(); navigate('/pasajero/buscar'); }}>Ver detalles</Button>
          ...
          <Card className="p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2.5">
              <StatCard compact icon="bi bi-clock-history" label="Viajes" value={String(average?.count ?? 0)} />
              <StatCard compact icon="bi bi-piggy-bank" label="Ahorrado" value={average ? `$${Math.round(average.count * 50)}` : '$0'} />
              <StatCard compact icon="bi bi-star" label="Rating" value={average?.average?.toFixed(1) ?? '—'} />
              <StatCard compact icon="bi bi-tree" label="CO2" value={`-${(average?.count ?? 0) * 2}kg`} />
            </div>
          </Card>
        </div>
