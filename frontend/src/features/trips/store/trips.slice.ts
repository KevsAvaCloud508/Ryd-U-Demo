...
export const searchTrips = createAsyncThunk('trips/search', async (params: TripSearchParams, { rejectWithValue }) => {
  try {
    return await fetchTrips(params);
  } catch (error) {
    return rejectWithValue(extractErrorMessage(error, 'No se pudieron buscar los viajes.'));
  }
});
