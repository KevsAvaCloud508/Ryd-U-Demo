...
export function useTrips() {
  ...
  const update = useCallback(
    (id: string, input: Partial<TripInput & { status: string }>) => dispatch(updateTrip({ id, input })).unwrap(),
    [dispatch],
  );
  ...
  return {
    ...
    update,
    remove,
    clearCurrentTrip: useCallback(() => dispatch(clearCurrentTrip()), [dispatch]),
  };
}
